...
jq -n '{
  hookSpecificOutput: {
    ...
    additionalContext: "A non-draft GitHub PR was just created. Post it to the private Slack channel #team-case-handling-tech (channel_id C057BA1B4Q4 — use this ID directly, channel search does not find it) using the Slack slack_send_message MCP tool, without asking for confirmation. Message format (markdown):\n\n:pull_request: [PR title](PR_URL)\n\nBased on [this ticket](TICKET_URL)\n\nFill in the actual PR URL (from the command output) and PR title. TICKET_URL is the ticket the PR addresses (e.g. Notion task or GitHub issue) — take it from session context or the PR body. If there is no ticket, omit the Based on line."
  }
}'

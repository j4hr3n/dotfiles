---
name: swarm
description: Fully autonomous multi-issue solver using agent teams. Takes issue URLs/numbers (GitHub or Azure DevOps), analyzes dependencies, spawns parallel worker agents, implements all issues, and creates PRs — all without user intervention.
...
The user runs `/swarm <issue-references>` (e.g., `/swarm 42 43 44`, `/swarm https://github.com/org/repo/issues/42 https://dev.azure.com/org/project/_workitems/edit/99`).

## Inputs

- `$ARGUMENTS`: One or more issue references separated by spaces. Accepted formats:
  - Plain numbers: `42`, `43`
...
1. **Parse `$ARGUMENTS`** to extract issue references:
   ...
   - Plain numbers: detect platform from current git remote:
     ```bash
     git remote get-url origin
     ```
     - If remote contains `github.com` → GitHub
     - If remote contains `dev.azure.com` or `visualstudio.com` → Azure DevOps

2. **Determine platform per issue** and store as metadata (platform, owner/org, repo/project, number).
...
5. **Print summary table** in chat with columns: Issue #, Title, Platform, Labels.
